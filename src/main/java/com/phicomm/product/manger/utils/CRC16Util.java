...
public class CRC16Util {
    ...
    private static final Logger logger = Logger.getLogger(CRC16Util.class);
     ...
     * @return CRC16校验值
     */
    public static int calcCrc16(byte[] data, int offset, int length) {
    ...
    public static void print(byte[] n) {
        for (byte aN : n) {
            StringBuilder t = new StringBuilder(Integer.toBinaryString(aN));
            for (int j = 0; j < 32; j++) {
                t.insert(0, "0");
            }
            System.out.print(t.substring(t.length() - 8));
            System.out.print("  ");
        }
        System.out.println();
    }
     ...
     * @return crc
     */
    public static int calcFileCrc16(File file) {
        byte[] buffer;
        try {
        ...
        return calcCrc16(buffer, 0, buffer.length);
    }
} 