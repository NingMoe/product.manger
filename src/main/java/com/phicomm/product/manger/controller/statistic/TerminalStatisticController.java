...
public class TerminalStatisticController {
     ...
     * 数据种类列表
     *
     * @param periodWithPlatformEntity 查询信息
     * @return 数据
     ...
     */
    @RequestMapping(value = "value/type/list", method = POST, consumes = "application/json", produces = "application/json")
    @ApiResponses(value = {
            @ApiResponse(code = 0, message = "正常情况", response = Response.class),
            @ApiResponse(code = 2, message = "数据格式错误", response = Response.class),
    ...
    @ApiOperation("获取某个平台、某个数据类型的数据种类")
    @ResponseBody
    public Response<List<String>> obtainValueTypeList(@RequestBody PeriodWithPlatformEntity periodWithPlatformEntity)
            throws TerminalStatisticTypeNotSupportException, PlatformNotExistException, DataFormatException {
        List<String> result = terminalStatisticService.obtainValueType(periodWithPlatformEntity);
        return new Response<List<String>>().setData(result);
    }
